...
3. Leave *"Provide user access to the AWS Management Console"* **unchecked** (this user is only for CLI and CI/CD).
4. Click **Next**.
...
2. In the search box, search for:
   ...
   AdministratorAccess
   ```
3. Check the checkbox next to **AdministratorAccess** *(provides necessary permissions to create Lambda functions, IAM roles, and configure public Function URLs)*.
...
5. Check the confirmation checkbox: *"I understand the above recommendation and want to proceed to create an access key."*
...
* If not installed, install it using Homebrew:
  ...
  brew install awscli
  ```
  *(Or download from [AWS CLI Official Installer](https://awscli.amazonaws.com/AWSCLIV2.pkg))*.
...
| `GEMINI_MODEL` | *(Optional)* Model ID | `gemini-3.5-flash-lite` |
...
* Real-time console logs are automatically sent to **AWS CloudWatch Logs** (under *Monitor → View CloudWatch logs* in the Lambda Console).
